...
def step_impl(context):
    ...
    response = requests.request("POST", context.uri, data=payload, headers=context.headers)
...
def step_impl(context):
    ...
    # print(response.text)
    response = requests.request("GET", url, headers=context.headers, params=querystring)
